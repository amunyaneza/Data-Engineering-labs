...
def extract(url, table_attribs):
    ...
    # loop and check the conditions (row should not be empty, first column should contain a hyperlink, and third column should not be '—' )
    for row in rows:
        col = row.find_all('td')
        if len(col)!=0:
            if col[0].find('a') is not None and '—' not in col[2]:
                ...
                #  pandas.concat() function to append the dictionary
...
def transform(df):
    ...
    #  Save dataframe column as a list
    ...
    # Type cast numerical text to float
    GDP_list = [float("".join(x.split(','))) for x in GDP_list]
    # Use numpy.round() function for rounding
    GDP_list = [np.round(x/1000,2) for x in GDP_list]
    # Assign modified list back to the dataframe
    df["GDP_USD_millions"] = GDP_list
    # Modify name of the column from 'GDP_USD_millions' to 'GDP_USD_billions'
...
#  The query statement to be executed is:
